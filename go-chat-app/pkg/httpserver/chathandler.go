...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	u := &userReq{}
	if err := json.NewDecoder(r.Body).Decode(u); err != nil {
		http.Error(w, "error decoding reqeust object", http.StatusBadRequest)
		return
	}
	res := login(u)
	json.NewEncoder(w).Encode(res)
...
func contactList(username string) *response {
	// if invalid usernaeme return error
	// if valid users fetch chats
	res := &response{}
	...
	if err != nil {
		...
		res.Message = "unable to fetch contact list please try later"
		return res
	}
