...
// this will require a Read and Wirte buffer size
...
// define a receiver which will lison for new message being sent to our Websocket
// endpoint
func receiver(client *Client) {
	for {
		// read in a message
		// readMessasge returns messageType, message err
	...
	for {
		...
		for client := range clients {
			...
			if client.Username == message.From || client.Username == message.To {
				...
				if err != nil {
					log.Println("Websocket error : %s", err)
					client.Conn.Close()
...
func setupRoutes() {
	...
	// map our `/ws` endpoint to the `severWs` function
